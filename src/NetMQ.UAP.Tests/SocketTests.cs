...
namespace NetMQ.Tests
{
    ...
    public class SocketTests
    {
        ...
        [TestMethod, TestCategory("Explicit")]
        ...
        public void Unbind(string protocol)
        {
            using (var server = new DealerSocket())
            {
                ...
                using (var client2 = new DealerSocket())
                {
                    ...
                    {
                        Assert.ThrowsException<EndpointNotFoundException>(() => { client2.Connect(address2); });
                    }
                }
            }
        }
        ...
        {
            {
                using (var dealer = new DealerSocket())
                {
                    ...
                    using (var router = new RouterSocket())
                    {
                        ...
                        Assert.ThrowsException<HostUnreachableException>(() => router.SendMoreFrame("dealer").SendFrame("Hello"));
                    }
                }
            }
        }
